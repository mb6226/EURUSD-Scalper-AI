...
def pivots_from_zigzag(data, depth, deviation, backstep):
    zz = zigzag_mt5(data["high"].to_numpy(), data["low"].to_numpy(), depth=depth, deviation=deviation, backstep=backstep)
...
def structural_metrics(data, pivots):
    names = ["pivots", "swings", "median_swing_pips", "median_bars", "trend_follow_rate", "reversal_follow_rate", "false_reversal_rate", "continuation_rate", "trend_rate", "range_rate", "trend_efficiency", "range_efficiency", "m5_alignment_rate"]
    if len(pivots) < 4:
    ...
    trend_mask, range_mask = roll >= 0.5, roll < 0.5
    m5c = data["close"].resample("5min").last().dropna()
    aligned = []
    for r in s.itertuples():
        ca = m5c.asof(pd.Timestamp(data.index[int(r.a)])); cb = m5c.asof(pd.Timestamp(data.index[int(r.b)]))
        if pd.isna(ca) or pd.isna(cb) or cb == ca:
            continue
        aligned.append((1 if cb > ca else -1) == r.direction)
    return {
        "pivots": float(len(pivots)), "swings": float(len(s)),
        "median_swing_pips": float(s["swing_pips"].median()), "median_bars": float(s["bars"].median()),
        "trend_follow_rate": float(1.0 - false_reversal), "reversal_follow_rate": float(reversal.mean()),
        "false_reversal_rate": false_reversal, "continuation_rate": continuation,
        "trend_rate": float(trend_mask.mean()), "range_rate": float(range_mask.mean()),
...
def main():
    ...
    for depth, deviation, backstep in pick_configs():
        ...
        rows.append({"depth": depth, "deviation": deviation, "backstep": backstep, **structural_metrics(data, pivots)})
    ...
    out.groupby("depth", as_index=False)[numeric].median().to_csv(RESULT_DIR / "structural_depth_summary.csv", index=False)
    out.groupby("depth", as_index=False)[["trend_follow_rate", "reversal_follow_rate", "false_reversal_rate", "continuation_rate", "trend_rate", "range_rate", "trend_efficiency", "range_efficiency", "m5_alignment_rate"]].median().to_csv(RESULT_DIR / "regime_summary.csv", index=False)
    out.groupby("depth").agg(configs=("depth", "size"), median_false_reversal=("false_reversal_rate", "median"), max_false_reversal=("false_reversal_rate", "max"), median_m5_alignment=("m5_alignment_rate", "median"), min_m5_alignment=("m5_alignment_rate", "min"), median_continuation=("continuation_rate", "median"), median_trend_rate=("trend_rate", "median"), median_range_rate=("range_rate", "median")).reset_index().to_csv(RESULT_DIR / "structural_robustness.csv", index=False)
    (RESULT_DIR / "README.md").write_text("# Step 5 — ZigZag Structural Regime Validation\n\nConsumes Step 3 quarterly candidates and validates structural behavior around Depth≈350. Descriptive only; no entry rule or PnL optimization.\n", encoding="utf-8")
